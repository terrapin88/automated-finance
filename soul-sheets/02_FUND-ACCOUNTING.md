...
- `systems_portfolio_accounting`
- `materiality_nav_bps`
- `materiality_usd`

## Inputs required
- GL trial balance (`systems_general_ledger`)
- Position data from portfolio accounting (`systems_portfolio_accounting`)
...
- NAV variance <`materiality_nav_bps`
...
## Escalation triggers
- Any reconciliation break >$10k or >`materiality_usd`
- NAV variance >`materiality_nav_bps` or >`materiality_usd`
...
## Tooling hooks
- Systems:
  - GL: `systems_general_ledger`
  - Portfolio accounting: `systems_portfolio_accounting`
  - Reporting: `systems_reporting`
- Files / folders:
...
1) **Daily Transaction Posting & Reconciliation**
   - Post GL entries from trades, cash, fees; reconcile positions.
2) **NAV Calculation & Validation**
   - Calculate NAV per policy; validate vs. prior periods; get approvals.
3) **Break Investigation & Resolution**
   - Document break; contact custodian/trader; trace root cause; post resolution entry.
4) **Period Close Checklist**
   - Run all GL subroutines, sign-off, prepare close package and evidence.
